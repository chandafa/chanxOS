...
if [[ -f "${ISO_NAME}" ]]; then
  ...
  echo "==> Build selesai, tetapi file ISO tidak ditemukan."
  echo "==> Isi file ISO yang ditemukan:"
  find . -maxdepth 3 -type f \( -name "*.iso" -o -name "*.hybrid.iso" -o -name "*.img" \) -print || true
  echo "==> Cek chanxos-build.log untuk detail."
  exit 1
fi
